...
Produs::Produs(Animal& an, string c, string n, double p , string di, string de, int nr)
{
	a = an;
	p_cod = c;
	p_nume = n;
...
{
	return a.toString() + " " + p_cod + " " + p_nume + " " + to_string(p_pret) + " " + p_data_in + " " + p_data_exp + " " + to_string(p_nr_exemplare);
}

string Produs::toStringDelimiter(char delim)
{
	return a.toStringDelimiter(delim) + delim + p_cod + delim + p_nume + delim + to_string(p_pret) + delim + p_data_in + delim + p_data_exp + delim + to_string(p_nr_exemplare);
}
